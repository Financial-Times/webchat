				...
				chatsession.delegate(".participant-option-edit", "click", function(e) {
					...
					editform.find("form").submit(function(e) {
						...
						apiRequest($.ajax({
							...
							data: {
								action: 'editmsg',
								messageid: msg.attr("data-mid"),
								newtext: encodeMessageForSendingToAPI(editform.find("textarea.msgtext").val()),
								keytext: keytext,
								isblockquote: (editform.find(".isblockquote input").get(0).checked ? 1 : 0)
							},
							xhrFields: {
								withCredentials: true
							},
